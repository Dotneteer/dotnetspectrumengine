...
namespace DotnetSpectrumEngine.Core.Devices.Sound
{
    ...
    public class BandPassFilter
    {
        ...
        public BandPassFilter(int numTaps, double freqSampling, double freqLow, double freqHigh)
        {
            _numTaps = numTaps;
            _taps = new double[numTaps];
            _sr = new double[numTaps];
            var lambda = Math.PI * freqLow / (freqSampling / 2);
            var phi = Math.PI * freqHigh / (freqSampling / 2);
